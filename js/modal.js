...
export default class Modal {
    constructor(container) {
        ...
        const formEnterTemplate =
            document.querySelector("#form-template").content;
        this.formWrap = formEnterTemplate
            .querySelector(".form")
            .cloneNode(true);
    ...
    enter() {
        const enterFillingTemplate = document.querySelector(
            "#form_enter-filling",
        ).content;
    ...
    visitEdit(visitObject) {
        ...
        this._renderVisitModal(
            "Відкоригуйте необхідну інформацію про візит",
            doctor,
            name,
            origins,
            purposeVisit,
        ...
        this.form.addEventListener("submit", (e) => {
            e.preventDefault();
            const visit = this._createVisitObject();
            Requests.put(visit, id)
                .then((visitEditObj) => {
                    this.formWrap.remove();
                    const { doctor } = visitEditObj;
                    const cardForEdit = cardContainer.querySelector(
                        `[data-id="${id}"]`,
                    );
                    const addInfo =
                        cardForEdit.querySelectorAll(".visit__add-info");
                    addInfo.forEach((doctorContainer) =>
                        doctorContainer.classList.remove("active"),
                    );
                    for (let key in visitEditObj) {
                        if (key !== "id") {
                            const elemForEdit = cardForEdit.querySelector(
                                `[data-edit="${key}"]`,
                            );
                    ...
                    if (doctor === "Стоматолог") {
                        cardForEdit
                            .querySelector(".visit__add-info-dentist")
                            .classList.add("active");
                        cardForEdit.style.backgroundColor =
                            "rgba(101, 235, 183, 0.5)";
                    } else if (doctor === "Кардіолог") {
                        cardForEdit
                            .querySelector(".visit__add-info-cardio")
                            .classList.add("active");
                        cardForEdit
                            .querySelector(".visit__add-info-age")
                            .classList.add("active");
                        cardForEdit.style.backgroundColor =
                            "rgba(205, 210, 497, 0.5)";
                    } else if (doctor === "Терапевт") {
                        cardForEdit
                            .querySelector(".visit__add-info-age")
                            .classList.add("active");
    ...
    _renderVisitModal(
        ...
        if (doctor === "") {
            this.selectDoctor.querySelector(`[value='Стоматолог']`).selected =
                "selected";
            dentistContainer.classList.add("active");
            dentistContainer.querySelector("input").required = true;
        } else {
            this.selectDoctor.querySelector(`[value='${doctor}']`).selected =
                "selected";
            switch (doctor) {
                case "Кардіолог":
        ...
        if (origins !== "") {
            this.origins.querySelector(`[value='${origins}']`).selected =
                "selected";
        }
