...
export class Visit {
    ...
    render(parent) {
        const { id, name, doctor, origins, purposeVisit, description } =
            this.data;
                ...
                if (ok) {
                    ...
                    if (renderedVisits.length === 0) {
                        cardContainer.textContent =
                            "Записів до лікарів на цей час немає";
...
export class VisitCardiologist extends Visit {
    constructor(data) {
        super(data);
        this.elem.cardioContainer = this.cardWrap.querySelector(
            ".visit__add-info-cardio",
        );
        this.elem.pressure = this.elem.cardioContainer.querySelector(
            ".visit__text-pressure",
        );
        this.elem.indexMassa = this.elem.cardioContainer.querySelector(
            ".visit__text-imassa",
        );
        this.elem.ill = this.elem.cardioContainer.querySelector(".visit__ill");
        this.elem.ageContainer = this.cardWrap.querySelector(
            ".visit__add-info-age",
        );
...
export class VisitDentist extends Visit {
    constructor(data) {
        super(data);
        this.elem.dentistContainer = this.cardWrap.querySelector(
            ".visit__add-info-dentist",
        );
        this.elem.lastVisit =
            this.elem.dentistContainer.querySelector(".visit__date");
    }
    render(parent) {
        super.render(parent);
